...
public class ShrinkIceWind : MonoBehaviour
{
    ...
    */
    ...
    void Start()
    {
        ...
        //GameObject.Find("UISpellManager").GetComponent<UserInterface>().hint = "grow";
    }

    // Update is called once per frame
    void Update()
    {
        if (avatar.transform.localPosition.x < 700f && avatar.transform.localPosition.x > 600f)
        {
            avatar.GetComponent<AvatarSpells>().canGrow = false;
        }
        /*
        if (avatar.transform.localPosition.x < 190f && avatar.transform.localPosition.x > 170f)
        {
            avatar.GetComponent<AvatarSpells>().canGrow = false;
        }
